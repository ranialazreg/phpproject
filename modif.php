...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Formulaire avec CSS intégré</title>
    <script>
...
a:hover{
  ...
  <div class='app-content'>
    ...
    <form action="" method="POST" enctype="multipart/form-data" style='background-color: #101827;border:none'>

    <h1 style='color:white'>Modification du Produit</h1>
    <br>

      <label style='color:white' >Designation  : </label>
      <input type="text" id="email" name="designation" style='color:white' required>

      <label for="motdepasse" style='color:white'>Prix :</label>
      <input type="number" id="motdepasse"  name="prix" style='color:white' required>

      <label for="file" style='color:white'>Choisir une image :</label>
		  <input type="file" name="file" id="file" style='color:white;background-color:white'><br>

      <a href="liste.php" style='color:white;font-size:1.3em' class="a">Liste des produits </a><br><br>
...
function formulaireSoumis() {
 ...
 if(formulaireSoumis()){
   ...
   $req1="UPDATE produit SET designation = '$design', prix = '$prix' , image='$img' WHERE id='$id' ";
   
   $res=$conn->query($req1);
   // ou en : $res=mysqli_query($conn,$req); 
   
   if( $res ){
    echo "<script> alert('Modification réussi !!!') ;</script>";
   }
   else{ echo "<script> alert('Echec Modification !!!') ;</script>"; }
